...
const programadesCount = computed(
    () => dates.value.filter((d) => d.estat === 'programada').length,
);
const urgentCount = computed(
    () => dates.value.filter((d) => d.urgencia === 'urgent').length,
);
...
<template>
    ...
    <h1 class="mx-6 mt-4 text-2xl font-semibold text-pmf-primary">Agenda</h1>
            ...
            <div class="rounded-xl border border-[#c5d8d5] bg-white p-5">
                ...
                <div v-if="todayDates.length" class="mt-4 space-y-4">
                    ...
                    >
                        ...
                        <button
                            v-if="date.estat !== 'cancel·lada'"
                            @click="openCancelModal(date.id)"
                            class="mt-2 w-full cursor-pointer rounded-lg bg-red-50 px-3 py-1.5 text-xs font-medium text-red-600 transition-colors hover:bg-red-100"
                        >
                            Cancel·lar cita
                        </button>
                    </div>
