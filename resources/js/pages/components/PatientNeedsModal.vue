...
const unassignedNeeds = computed(() =>
    props.availableNeeds.filter(
        (n) => !localNeeds.value.some((ln) => ln.id === n.id),
    ),
);
...
<template>
    ...
    >
        <div
            class="mx-4 w-full max-w-md overflow-hidden rounded-2xl border border-[#b0ceca] bg-white shadow-xl"
        >
            <div
                class="flex items-center justify-between border-b border-[#deecea] bg-[#f0f7f6] px-6 py-4"
            >
                <div>
                    <h3 class="text-[15px] font-semibold text-pmf-green-dark">
                        Gestionar necessitats
                    </h3>
                    <p class="mt-0.5 text-xs text-pmf-grey-light">
                        {{ patient.name }}
                    </p>
                </div>
                <button
            ...
            <div class="max-h-[50vh] space-y-2 overflow-y-auto px-6 py-4">
                <div
                    v-if="localNeeds.length === 0 && !showSelect"
                ...
                >
                    <span
                        class="h-1.5 w-1.5 flex-shrink-0 rounded-full bg-pmf-primary"
                    ></span>
                    <span class="flex-1 text-sm text-pmf-green-dark">{{
                        need.name
                    }}</span>
                    <button
                        @click="removeNeed(need.id)"
                        :disabled="loading"
                        class="cursor-pointer rounded-lg p-1.5 text-pmf-grey-light transition-colors hover:bg-red-50 hover:text-red-500 disabled:opacity-40"
                        aria-label="Eliminar necessitat"
                    >
                ...
                >
                    ...
                    >
                        <option :value="null" disabled>
                            Selecciona una necessitat...
                        </option>
                        <option
                            v-for="n in unassignedNeeds"
                            :key="n.id"
                            :value="n.id"
                        >
                            {{ n.name }}
                    ...
                    <button
                        ...
                        class="btn-primary w-full px-3 py-1.5 text-xs sm:w-auto"
                    ...
                    <button
                        ...
                        class="btn-secondary w-full px-3 py-1.5 text-xs sm:w-auto"
                    >
                        Cancel·lar
                    </button>
                </div>
            ...
            <div
                class="flex items-center justify-between border-t border-[#deecea] bg-[#f9fcfc] px-6 py-3"
            >
                <button
                    @click="
                        showSelect = true;
                    ...
                    class="inline-flex cursor-pointer items-center gap-1.5 text-xs font-medium text-pmf-green transition-colors hover:text-pmf-primary disabled:opacity-40"
                >
...
.btn-secondary:hover {
    background-color: var(--pmf-secondary);
}
</style>
