...
const doctorName = computed(
    () => props.date?.worker?.user?.name || 'No assignat',
);
...
watch(
    ...
    (currentDate) => {
        ...
        estimatedMinutes.value =
            typeof currentDate.time === 'number' ? currentDate.time : null;
        resetSlotSelection();
        slotsMessage.value = '';
        saveMessage.value = '';
        saveStatus.value = '';
...
watch(
    ...
    (isOpen) => {
        ...
        if (estimatedMinutes.value === null) {
            estimatedMinutes.value =
                typeof props.date.time === 'number' ? props.date.time : null;
        }
...
async function submitReschedule() {
    if (!props.date?.id || !selectedDateTime.value) {
        ...
        saveMessage.value =
            'Selecciona una hora disponible abans de canviar la data.';
    ...
    try {
        ...
        if (!response.ok || payload?.status === 'error') {
            saveStatus.value = 'error';
            saveMessage.value =
                payload?.message || "No s'ha pogut reprogramar la cita.";
        ...
        saveMessage.value =
            payload?.message || 'Cita reprogramada correctament.';
        closeModal();
    } catch {
        saveStatus.value = 'error';
...
async function checkAvailability() {
    if (
        ...
        resetSlotSelection();
        slotsMessage.value =
            'Selecciona data abans de veure franges disponibles.';
        saveMessage.value = '';
        saveStatus.value = '';
    ...
    try {
        ...
        selectedStartTime.value = startTimeOptions.value.includes(
            preferredStartTime,
        )
            ? preferredStartTime
    ...
    <Teleport to="body">
        ...
        >
            ...
            >
                <!--header -->
                <div
                    class="sticky top-0 flex items-center justify-between rounded-t-lg border-b border-gray-200 bg-white px-6 py-4"
                >
                    <div>
                ...
                <div class="space-y-6 p-6">
                    <div class="rounded-lg border border-gray-200 bg-white p-4">
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            Selecciona un dia, comprova les franges disponibles
                            i escull una hora.
                        </p>
                    </div>

                    <div
                        class="grid gap-4 sm:grid-cols-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] md:items-end"
                    >
                        <div class="w-full">
                            <label
                                for="reschedule-date"
                            ...
                            <input
                                ...
                                class="mt-3 h-9 w-full rounded-md border border-gray-300 bg-white px-3 text-sm text-gray-900 focus:border-pmf-primary focus:ring-2 focus:ring-pmf-primary/30 focus:outline-none"
                                autocomplete="off"
                                required
                                aria-required="true"
                            />
                    ...
                    <div
                        class="rounded-md border border-pmf-primary/20 bg-white p-4"
                    >
                        <p
                            id="slots-heading"
                ...
                <!--footer -->
                <div
                    class="sticky bottom-0 flex justify-end gap-3 rounded-b-lg border-t border-gray-200 bg-gray-50 px-6 py-4"
                >
                    <button
                        @click="closeModal"
                        class="rounded-lg border border-pmf-primary/40 bg-white px-4 py-2 text-pmf-primary transition-colors hover:bg-pmf-secondary"
                    >
                        Tancar
                    </button>
                    <button
                        ...
                        class="rounded-lg bg-pmf-primary px-4 py-2 text-white transition-colors hover:bg-pmf-green disabled:cursor-not-allowed disabled:opacity-60"
                    >
