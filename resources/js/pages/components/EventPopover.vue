...
const getUrgencyStyle = (urgencia: string) => {
    switch (urgencia) {
        ...
        case 'preferent':
            return { bg: 'bg-orange-100 text-orange-700', label: 'Preferent' };
        default:
            return { bg: 'bg-[#f0f7f6] text-pmf-green', label: 'No urgent' };
    }
};
...
const getStatusStyle = (estat: string) => {
    switch (estat) {
        case 'programada':
            return { bg: 'bg-blue-100 text-blue-700', label: 'Programada' };
        case 'realitzada':
            return { bg: 'bg-[#f0f7f6] text-pmf-green', label: 'Realitzada' };
        case 'cancel·lada':
            return { bg: 'bg-red-100 text-red-700', label: 'Cancel·lada' };
        default:
            return { bg: 'bg-gray-100 text-gray-600', label: estat };
    }
};
...
<template>
    <div v-if="visible && event" class="pointer-events-none fixed inset-0 z-50">
        <!-- Close popover -->
        <div
        ...
        <!-- Popover -->
        <div
            class="event-popover pointer-events-auto fixed z-50"
        ...
        >
            ...
            <div class="mb-3 flex items-start justify-between gap-3">
                <div class="flex items-center gap-2">
                    ...
                    ></span>
                    <p class="text-sm font-medium text-pmf-green-dark">
                        {{ event.title }}
                    </p>
                </div>
                <button
                    @click="emit('close')"
                    aria-label="Tancar"
                    class="flex-shrink-0 cursor-pointer rounded p-0.5 text-pmf-grey-light hover:bg-pmf-secondary"
                >
            ...
            <!-- Date -->
            <div
                class="mb-3 flex items-center gap-2 text-xs text-pmf-grey-light"
            >
            ...
            <div class="space-y-2 text-xs">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span
                        :class="[
                            ...
                            getStatusStyle(event.extendedProps.status).bg,
                        ]"
                    >
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span
                        :class="[
                            'rounded-full px-2 py-0.5 font-medium',
                            getUrgencyStyle(event.extendedProps.urgency).bg,
                        ]"
                    >
            ...
            >
                <button
                    ...
                    class="w-full cursor-pointer rounded-lg bg-red-50 px-3 py-1.5 text-xs font-medium text-red-600 transition-colors hover:bg-red-100"
                >
                    Cancel·lar cita
                </button>
            </div>
