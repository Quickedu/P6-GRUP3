...
<template>
    ...
    >
        <div
            v-if="visible"
            class="fixed right-4 top-4 z-[9999] w-96 max-w-[calc(100vw-2rem)] pointer-events-auto"
        >
            <div
            ...
            >
                <span
                    class="rounded px-2 py-0.5 text-xs font-semibold uppercase tracking-wide whitespace-nowrap flex-shrink-0"
                    :class="statusBadgeClasses"
                >
                    {{ status }}
                </span>

                <p class="flex-1 min-w-0 text-sm leading-5" :class="statusTextClasses">
                    {{ message }}
                </p>
