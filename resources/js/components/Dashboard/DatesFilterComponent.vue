...
const normalizeSearch = (value: string): string =>
    value.trim().toLocaleLowerCase('ca-ES');
...
const selectedDoctorId = computed<number | null>(() => {
    ...
    const doctorByName = props.doctors.find(
        (doctor) => normalizeSearch(doctor.name) === normalizeSearch(search),
    );

    return doctorByName?.id ?? null;
});

const hasInvalidDoctorSearch = computed(
    () => doctorSearch.value.trim() !== '' && selectedDoctorId.value === null,
);
...
<template>
    <div
        class="mb-3 flex w-full flex-col gap-3 rounded-lg border border-b border-[#c5d8d5] bg-[#f0f7f6] p-2"
    >
        ...
        <button
            ...
            type="button"
            @click="clearFilters"
            class="flex cursor-pointer items-center gap-1 rounded-lg px-1 pt-1 text-sm font-medium text-pmf-primary transition hover:bg-gray-100"
        >
            <X class="h-4 w-4" />
            Esborrar filtres
        </button>
        ...
        <div class="flex items-center justify-between gap-2">
            <button
                type="button"
                @click="previousDay"
                class="flex cursor-pointer items-center gap-2 rounded-lg bg-pmf-green py-2 pr-2 pl-1 text-sm font-medium text-white transition hover:bg-pmf-primary"
            >
            ...
            <div class="flex flex-col items-center">
                <label for="selecciona-data" class="mb-1 text-xs text-gray-600"
                ...
                <input
                    ...
                    class="cursor-pointer rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-pmf-primary focus:outline-none"
                />
            </div>
            ...
            <button
                type="button"
                @click="nextDay"
                class="flex cursor-pointer items-center gap-2 rounded-lg bg-pmf-green py-2 pr-1 pl-2 text-sm font-medium text-white transition hover:bg-pmf-primary"
            >
                Següent
        ...
        <div class="flex flex-col">
            <label for="doctor-id" class="mb-1 text-xs text-gray-600"
                >Filtra per doctor/tècnic</label
            >
            <input
                id="doctor-id"
                ...
                class="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-pmf-primary focus:outline-none"
            />
        ...
        <button
            ...
            :disabled="isLoading"
            class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg bg-pmf-primary px-3 py-2 text-sm font-medium text-white transition hover:bg-pmf-green disabled:cursor-not-allowed disabled:opacity-50"
        >
