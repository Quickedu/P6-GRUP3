...
const handleSearch = async () => {
    if (!isValidNts.value) {
        error.value =
            'El NTS ha de tenir 4 lletres majúscules i 10 dígits, sense espais.';
    ...
    try {
        const response = await fetch(
            filterPatientDates.url({ query: { nts: nts.value } }),
        );
...
<template>
    <div class="space-y-6">
        <div class="flex flex-col gap-4">
            <h2 class="text-lg font-bold">Filtrar cites per pacient</h2>

            <div class="flex gap-2">
                <div class="flex-1">
                    <label
                        for="nts"
                        class="sr-only mb-1 block text-sm font-medium text-gray-700"
                    >
                        NTS del pacient
                    </label>
                    <input
                        ...
                        class="w-full cursor-pointer rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-pmf-primary focus:outline-none"
                        maxlength="14"
                    />
                    <p
                        v-if="nts && !isValidNts"
                ...
                <button
                    ...
                    class="flex cursor-pointer items-center gap-2 rounded-lg bg-pmf-green px-2 text-sm font-medium text-white transition hover:bg-pmf-primary"
                >
